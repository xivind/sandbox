...
class Strava:
    ...
    def __init__(self, oauth_file, dates):
        self.token = dict()
        self.extra = dict()
        self.json_response = ""
        self.before_date_epoch = dates[1].timestamp()
        self.after_date_epoch = dates[0].timestamp()
    ...
    def get_data(self):
        ...
        protected_url = f"https://www.strava.com/api/v3/athlete/activities?before={self.before_date_epoch}&after={self.after_date_epoch}&page=1&per_page=200"
...
def get_dates():
    """Method to get dates as input to Strava API call"""
    after_date_input = input(f'{datetime.now().strftime("%d.%m.%Y %H:%M:%S")}: Get activities after (format YYYY-MM-DD): ')
    before_date_input = input(f'{datetime.now().strftime("%d.%m.%Y %H:%M:%S")}: Get activities before (format YYYY-MM-DD): ')

    try:
        after_date_object = datetime.strptime(after_date_input, "%Y-%m-%d")
        before_date_object  = datetime.strptime(before_date_input, "%Y-%m-%d")

        if before_date_object < after_date_object:
            print(f'{current_time()}: to date is before from date...')
            date_list = ""
            return date_list

        else:
            full_date_list = pendulum.period(after_date_object+timedelta(minutes=1439),before_date_object)
            tmp_date_list = []
            for dates in full_date_list:
                tmp_date_list.append(dates)
            last_date = len(tmp_date_list)
            date_list = []
            date_list.insert(0, tmp_date_list[0])
            date_list.insert(1, tmp_date_list[last_date-1])
            return date_list

    except Exception:
        print(f'{current_time()}: Unspecified errors in dates')
        date_list = ""
        return date_list
...
if __name__ == "__main__":
    ...
    cutoff_dates = get_dates()

    strava = Strava(PARAMETERS.oauth_file, cutoff_dates)
